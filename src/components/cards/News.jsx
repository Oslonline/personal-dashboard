...
export default function News({ countryCode }) {
    ...
    return (
        <div className="col-span-3 row-span-3 row-start-3 flex flex-col rounded-xl bg-gradient-to-br from-zinc-950/80 to-zinc-950/40 p-5 pr-3 backdrop-blur-xl">
            <h3 className="pb-2 text-lg font-semibold">Last News</h3>
            <ul className="flex touch-pan-y flex-col gap-2 overflow-y-auto overflow-x-hidden overscroll-contain pr-2">
                {news.map((article, index) => (
                    <a className="group flex items-center justify-between rounded-xl p-2 duration-100 hover:bg-gray-50/10" key={index} href={article.url} target="_blank" rel="noopener noreferrer">
